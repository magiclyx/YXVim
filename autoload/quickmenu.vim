...
function! s:window_open(size)
	...
	if stridx(g:quickmenu_options, 'T') < 0
		...
		echom "silent! bortright ".size.'split '.s:quickmenu_name
		exec "silent! bortright ".size.'split '.s:quickmenu_name
	else
		"exec "silent! vsplit ".size.'split '.s:quickmenu_name
		exec "silent! bortright ".size.'split '.s:quickmenu_name
	endif
	if savebid == bufnr('%')
		return 0
...
function! quickmenu#append(text, event, ...)
	...
	let item.help = help
	if a:event != ''
        " mode 0 is active item
		let item.mode = 0
	elseif a:text[0] != '#'
        " mode 1 is title
		let item.mode = 1
	else
        " mode 2 is normal text
		let item.mode = 2
